...
public:
	...
	void ResetScore() { mScore = SCORE_NOT_EVALUATED; mEvaluationResults.clear(); mBestTejun.clear(); mTejunJap.clear(); } 
	int GetParentNodeID() const { return mParentNodeID; }
	void SetEvaluations(const vector<EvaluationResult>& evaluations);
	const vector<EvaluationResult>& GetEvaluations() const { return mEvaluationResults; }
...
//		wfs << wstring(mState.begin(), mState.end()) << endl;
		...
		for (const auto& evaluation : mEvaluationResults)
		{
			wfs << evaluation.score << endl;
			wfs << wstring(evaluation.tejun.begin(), evaluation.tejun.end()) << endl;
		}
	}
...
//		GetLineTrim(wfs, ws);	mState = string(ws.begin(), ws.end());
		...
		if (version >= 3)
		{
			int numEvaluationResults = 0;
			GetLineTrim(wfs, ws);	numEvaluationResults = stoi(ws);
			mEvaluationResults.reserve(numEvaluationResults);
			for (int i = 0; i < numEvaluationResults; ++i)
			{
				EvaluationResult evaluation;
				GetLineTrim(wfs, ws);	evaluation.score = stoi(ws);
				GetLineTrim(wfs, ws);	evaluation.tejun = string(ws.begin(), ws.end());
				mEvaluationResults.push_back(evaluation);
			}
		}

		if (mEvaluationResults.empty() && IsScoreEvaluated())
		{
			EvaluationResult evaluation;
			evaluation.score = mScore;
			evaluation.tejun = mBestTejun;
			mEvaluationResults.push_back(evaluation);
		}
	}
	...
	vector <EvaluationResult> mEvaluationResults;
...
public:
	...
	void UpdateNode(int nodeID, const vector<Node::EvaluationResult>& evaluations, bool isMate=false);
